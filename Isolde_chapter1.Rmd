...
rasterToMatch <- raster(paste0(getwd(),"/LCC2005_V1_4a.tif")) #LandR::prepInputsLCC() 
...
Multiple rasters of predicted bird density are downloaded, postProcessed, loaded and outputted as a raster stack. To do this we first define a function to download rasters for the chosen bird species, and output a list of the rasters that have been downloaded. 
...
birdRaster <- birdDensityRasterStack$OVEN

landBirdRasterStack <- stack(landscapeRaster, birdRaster) # add landscape raster to birdDensityRasterStack
cellValues <- as.data.frame(getValues(landBirdRasterStack)) # take the values from the rasters and input them to a dataframe called cellValues
...
plotBirdsByClass <- ggplot(data = birdsByClass, aes(x =landCoverClass, y = meanBirdDensity)) + geom_bar(stat="identity") +
...
plotVarBirdDensity <- ggplot(data = birdsByClass, aes(x =landCoverClass, y = varBirdDensity)) + geom_bar(stat="identity")
plotVarBirdDensity
