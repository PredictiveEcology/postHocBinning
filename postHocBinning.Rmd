...
Here the LLC2005 raster layer of Canada is read in using the LandR prepInputs function. It will be used as the rasterToMatch layer for other spatial layers to reproject to, and will also provide land cover classification data.
...
### Get multiple bird rasters - 1km bootstrapped rasters
...
```{r functionDownload1kmBootRasters}
# #load/download necessary packages
#   reproducible::Require("googledrive")
#   reproducible::Require("raster")
#   
# download1kmBootRasters <- function(folderUrl, 
#                                        birdsList, 
#                                        rastersPath){
# #drive_ls function is used to list all the files it finds using the folder url with the given pattern
#    filesToDownload <- googledrive::drive_ls( path = as_id(folderUrl), pattern = "pred-") #note: has to be changed if filenaming system changes
...
#    X <- lapply(rastersForBirdList, substr, 6, 9) #works for strings of the form pred-XXXX-CAN-Mean
...
```{r functionLoad1kmBootRasters}
# 
# load1kmBootRasters <- function(birdsList,
#                                    folderUrl,
#                                    rastersPath,
#                                    rasterToMatch,
#                                    studyArea){
...
#   downloadedRasters <- download1kmBootRasters(folderUrl = folderUrl,  
#                                                   birdsList = birdsList, 
...
# downloadFolderBird <- paste0(getwd(),"/birdRasterFiles") #say where to download to
# folderUrlBird <- "https://drive.google.com/drive/folders/1exWa6vfhGo1DNUL4ei2baDz77as7jYzY?usp=sharing" # give file location 
# 
# birdDensityRasterStack <- load1kmBootRasters(folderUrl = folderUrlBird,
#                                                  birdsList = birdsList,
...
download250mBootRasters <- function(folderUrl,
                                       birdsList,
                                       rastersPath){
#drive_ls function is used to list all the files it finds using the folder url with the given pattern
...
downloadFolderBird <- paste0(getwd(),"/birdRasterFiles") #say where to download to
folderUrlBird <- "https://drive.google.com/drive/folders/1fCTr2P-3Bh-7Qh4W0SMJ_mT9rpsKvGEA?usp=sharing" # give file location 

birdDensityRasterStack <- load250mBootRasters(folderUrl = folderUrlBird,
                                                 birdsList = birdsList,
                                                 rastersPath = downloadFolderBird,
                                                 rasterToMatch = landscapeRaster,
                                                 studyArea = studyArea)

noBootsDFLocation <- "https://drive.google.com/file/d/1ldESo9gb6icRD8ZsuPgaEDSIwFjJEe4W/view?usp=sharing"
noBootsDF <- drive_download(file = noBootsDFLocation, 
                            path = paste0(downloadFolderBird, "/noBootsDF"),
                            type = "spreadsheet",
                            overwrite = TRUE)
...
Here the data for each cell's mean bird density and variance between bootstrap replicates is collecte in a data table. 
