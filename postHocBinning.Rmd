  ...
  birdStatsByClass <- lapply(X = birdDatasets,
                             FUN = function(singleBirdDataset){
                               ...
                               birdStats <- singleBirdDataset[order(landCoverClass) # order the rows by the land cover class
                                              ...
                                              normality = ad.test(birdDensity)$p.value,
...
```{r functionGetBirdAssumptionsSummary}

getBirdAssumptionsSummary <- function(birdStatsTables){
  
  namesBirdsAnalysed <- names(birdStatsTables) 
  
  birdAssumptionsSummary <- lapply(X = birdStatsTables,
                             FUN = function(singleBirdStats){
          
           landCoverClass <- dplyr::pull(singleBirdStats, landCoverClass) 
                               
                normality <- dplyr::pull(singleBirdStats, normality)
            normalityTrue <- ifelse(normality <= 0.05,"yes","no")
          
              unimodality <- dplyr::pull(singleBirdStats, unimodality)
          unimodalityTrue <- ifelse(unimodality <= 0.05,"yes","no")
                           
        singleBirdAssumptionsSummary <- data.table(landCoverClass, normalityTrue, unimodalityTrue)
                                           
              return(singleBirdAssumptionsSummary)
            })
  
  names(birdAssumptionsSummary) <- namesBirdsAnalysed
  
        return(birdAssumptionsSummary)
}

```


```{r getBirdAssumptionsSummary}

birdAssumptionsSummary <- getBirdAssumptionsSummary(birdStatsTables = birdStatsByClass)
...
plotsBirdStatsByClass <- lapply(X = birdStatsByClass,
    ...
    plotBirdStatsByClass <- ggplot(data = singleBirdStats, aes(x =landCoverClass, y = meanBirdDensity)) +   geom_bar(stat="identity", width=0.7, fill="steelblue") + theme_classic() + ggtitle("Mean Bird Density by Cover Class, with SE Bars") + theme(axis.text=element_text(size=7)) +
geom_errorbar(aes(ymin=meanBirdDensity-seBirdDensity, ymax=meanBirdDensity+seBirdDensity), width=.2) 
 
return(plotBirdStatsByClass)
...
densityPlot <- plot(birdCoverDensity,
     main = paste0("Kernel Density Plot for ", paste0(birdName), " in Cover Class ", coverType),
...
kernelDensityPlot <- getKernelDensityPlot(birdName = "Ovenbird", #sp name for title
                                          coverType = "2", #cover type name for title
                                          birdCoverDensity = kernelDensityData$OVEN$"2", #which kernel density cover type and bird species to plot
                                          meanData = birdStatsByClass$OVEN[2,meanBirdDensity] #show a red line with the mean density
                                          )
