...
getBirdStatsByClass <- function(birdDatasets) {
  ...
  birdStatsByClass <- lapply(X = birdDatasets, FUN = function(singleBirdDataset) {
    ...
    birdStats <- singleBirdDataset[order(landForClass) # order the rows by the land cover class
...
- contact Ana to get numeric ages, not categorical.
- filter out non-forested types from input table, resolve factor level issue so non-forested are not included
- for the test-case we can fit brt with n = 1400000, but may take time if it is more. 
- then can fit gbm: gbm::gbm(formula = birdDensity ~ landForClass + age, distribution = "gaussian", data = birdDatasets$meanOVEN)
...
- package results - matrix, age class bins metadata, 1D binning results for non-forest types

```{r fitBRT }

#separate out data table rows that are forested
ForOvenDF <- birdDatasets$meanOVEN[forestedStatus == "1"]

#fit gbm
birdData <- ForOvenDF
gbmFitted <- gbm::gbm(formula = birdDensity ~ landForClass + age, distribution = "gaussian", data = birdData)

#generate prediction df using expand(?)
birdPredictDF <- expand(birdData)
